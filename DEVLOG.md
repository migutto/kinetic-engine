...
## 2026-04-04 - Documentation sync after PWA hardening

This pass did not change runtime behavior in the app itself. Its purpose was to bring all Markdown documentation back in line with the real product state after the recent wave of feature, UX, summary, and PWA work.

### Highlights

- Refreshed `README.md` so it now reflects the current feature set:
  - distance-first cardio logging
  - training date backfill
  - weekly/monthly summaries
  - active PWA architecture
- Rewrote backlog and strategy docs so completed work is no longer presented as future roadmap.
- Updated the user-facing shortcut notes in `devshortcut.md` with a simpler summary of the latest changes.
- Refreshed `CHANGELOG.md` to match the current app behavior and current PWA stack.

### Why this mattered

The app has moved quickly over the last few iterations. Without a documentation sync, the repo would continue to describe an older product than the one the code now delivers. This pass closes that gap and makes the docs more useful for future planning, onboarding, and release notes.
